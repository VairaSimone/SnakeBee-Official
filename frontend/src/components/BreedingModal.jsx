...
return (
  <div
    className="fixed inset-0 bg-black/50 flex justify-center items-center p-2 sm:p-4 z-50"
    onClick={onOverlayClick}
    style={{ cursor: 'pointer' }}
  >
    <div
      ref={modalRef}
      className="bg-white text-black rounded-xl p-4 sm:p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-lg"
      style={{ cursor: 'auto' }}
    >
      <button onClick={handleClose} className="float-right text-2xl">×</button>
      <h3 className="text-xl sm:text-2xl font-bold mb-4">Nuova Riproduzione</h3>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 mt-4" noValidate>
        ...
        <div>
          ...
          {events.map((field, idx) => (
            <div key={field.id} className="flex flex-col sm:flex-row items-start sm:items-end gap-2 mb-2">
              <select
                {...register(`events.${idx}.type`)}
                disabled={!seasonOpen}
                className="bg-white text-black border p-2 rounded w-full sm:w-auto"
              >
              ...
              <input
                ...
                className="bg-white text-black border p-2 rounded w-full sm:w-auto cursor-pointer"
              ...
              {seasonOpen && (
                <button
                  ...
                  className={`text-red-600 text-xl ${field.type === 'pairing' ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  ✕
                </button>
              )}
          ...
          {seasonOpen && (
            <select
              ...
              className="bg-white text-black mt-2 border p-2 rounded w-full sm:w-auto"
            >
        ...
        {hasBirthEvent && (
          <div>
            ...
            {hatchlings.map((f, i) => (
              <div key={f.id} className="flex flex-col sm:flex-row items-start sm:items-end gap-2 mb-2">
                <input
                  placeholder="Morph"
